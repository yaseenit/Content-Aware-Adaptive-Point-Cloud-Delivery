...
def eval(test_sequences,splits,pred):
    ...
    # open each file, get the tensor, and calculate the metrics
    num_errors = 0
    num_points = 0
    for scan_file, label_file, pred_file in zip(scan_names, label_names, pred_names):
        ...
        error = u_label_sem - u_pred_sem
        num_errors += np.count_nonzero(error)
        num_points += u_label_sem.shape[0]

    # when I am done, print the evaluation
    m_accuracy = evaluator.getacc()
    m_jaccard, class_jaccard = evaluator.getIoU()
    class_precision = evaluator.getprec(mean=False)
    class_recall = evaluator.getrec(mean=False)
    class_f1 = evaluator.getf1(mean=False)
    conf = evaluator.getconf()

    m_errors = 0
    if num_points != 0:
        m_errors = num_errors / num_points

    print('{split} set:\n'
          'Acc avg {m_accuracy:.3f}\n'
          'Prec avg {m_precision:.3f}\n'
          'Rec avg {m_recall:.3f}\n'
          'F1 avg {m_f1:.3f}\n'
          'IoU avg {m_jaccard:.3f}\n'
          'Error avg {m_errors:.6f}\n'.format(split=splits,
                                           m_accuracy=m_accuracy,
                                           m_jaccard=m_jaccard,
                                           m_errors=m_errors,
                                           m_precision=class_precision.mean(),
                                           m_recall=class_recall.mean(),
                                           m_f1=class_f1.mean()))

    save_to_log(FLAGS.predictions,'pred.txt','{split} set:\n'
          'Acc avg {m_accuracy:.3f}\n'
          'IoU avg {m_jaccard:.3f}\n'
          'Error avg {m_errors:.6f}'.format(split=splits,
                                           m_accuracy=m_accuracy,
                                           m_jaccard=m_jaccard,
                                           m_errors=m_errors))
    # print also classwise
    csv_summary = ""
    csv_summary += f"id ; name                    ; iou  ; pre  ; rec  ; f1   \n"

    for i in range(len(class_jaccard)):
        jacc = class_jaccard[i]
        ...
        if i not in ignore:
            ...
            row = f"{i:3d}; {class_str:24s}; {jacc:.3f}; {prec:.3f}; {rec:.3f}; {f1:.3f}\n"
            csv_summary+=row
    save_to_log(FLAGS.predictions, 'pred.txt', csv_summary)
    print(csv_summary)
    
    print("confusion matrix (cols = gt, rows = pred):")
    print("          ", end="")
    for c in range(conf.shape[0]):
        print(f"{c:10d}", end="")
    print("\n")

    for c, row in enumerate(conf):
        print(f"{c:10d}", end="")
        for col in row:
            print(f"{int(col.item()):10d}", end="")
        print("\n")
