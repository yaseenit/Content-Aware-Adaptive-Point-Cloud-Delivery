...
class iouEval:
    ...
    def getprec(self, mean=False):
        tp, fp, fn = self.getStats()
        prec = tp[self.include] / (tp[self.include] + fp[self.include] + 1e-15)
    ...
    def getrec(self, mean=False):
        tp, fp, fn = self.getStats()
        rec = tp[self.include] / (tp[self.include] + fn[self.include] + 1e-15)
    ...
    def getf1(self, mean=False):
        prec = self.getprec(mean=False)
        rec  = self.getrec(mean=False)
