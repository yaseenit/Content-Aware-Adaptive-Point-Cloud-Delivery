...
from tasks.semantic.modules.ClassDifferenceLoss import ClassDifferenceLoss
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None,uncertainty=False,class_difference_loss=False, disable_wce=False):
        # parameters
        self.ARCH = ARCH
        self.DATA = DATA
        ...
        self.class_difference_loss = class_difference_loss
        self.disable_wce = disable_wce
        ...
        self.ls = Lovasz_softmax(ignore=0, label_type='class').to(self.device)
        
        if self.class_difference_loss == 'iou':
            self.cdl = Lovasz_softmax(ignore=0, label_type=self.DATA["label_type"]).to(self.device)
        if self.class_difference_loss == 'me':
            self.cdl = ClassDifferenceLoss(ignore=0, label_type=self.DATA["label_type"], normalize=False, absolute=False).to(self.device)
        if self.class_difference_loss == 'me-relu':
            self.cdl = ClassDifferenceLoss(ignore=0, label_type=self.DATA["label_type"], normalize=False, absolute=False, activation='relu').to(self.device)
        if self.class_difference_loss == 'mae':
            self.cdl = ClassDifferenceLoss(ignore=0, label_type=self.DATA["label_type"], normalize=False, absolute=True).to(self.device)
        ...
        if self.n_gpus > 1:
            ...
            if self.class_difference_loss:
                self.cdl = nn.DataParallel(self.cdl).cuda()
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(train_loader):
            ...
            if self.uncertainty:
                ...
                jacc = self.ls(output_mean, proj_labels.long())
                wce = criterion(output_mean.clamp(min=1e-8), proj_labels)
                if self.disable_wce:
                    wce *= 0
                loss_m = wce + hetero + jacc
                ...
                wce = criterion(torch.log(output.clamp(min=1e-8)), proj_labels)
                if self.disable_wce:
                    wce *= 0
                loss_m = wce + jacc
            
            if self.class_difference_loss:
                loss_m += self.cdl(output, proj_labels)
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(val_loader):
                ...
                if self.class_difference_loss:
                    cdl = self.cdl(log_out, proj_labels)
                    loss += cdl
