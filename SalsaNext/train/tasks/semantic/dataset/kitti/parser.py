...
class Parser():
  ...
  def get_n_classes(self, learning_map_inv=None):
    if learning_map_inv is None:
      learning_map_inv = self.learning_map_inv
    return len(learning_map_inv)

  def get_original_class_string(self, idx, labels=None):
    if labels is None:
      labels = self.labels
    return labels[idx]

  def get_xentropy_class_string(self, idx, learning_map_inv=None):
    if learning_map_inv is None:
      learning_map_inv = self.learning_map_inv
    return self.labels[learning_map_inv[idx]]

  def to_original(self, label, learning_map_inv=None):
    # put label in original values
    if learning_map_inv is None:
      learning_map_inv = self.learning_map_inv

    return SemanticKitti.map(label, learning_map_inv)

  def to_xentropy(self, label, learning_map=None):
    # put label in xentropy values
    if learning_map is None:
      learning_map = self.learning_map
    return SemanticKitti.map(label, learning_map)

  def to_color(self, label, learning_map_inv=None, color_map=None):
    # put label in original values
    if learning_map_inv is None:
      learning_map_inv = self.learning_map_inv
    label = SemanticKitti.map(label, learning_map_inv)
  
    # put label in color
    if color_map is None:
      color_map = self.color_map
    return SemanticKitti.map(label, color_map)